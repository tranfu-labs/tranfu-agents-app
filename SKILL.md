...
**一个人多个 agent**:用户会对**每个 agent 各说一句**。每次只登记**当前这个** agent;
`operator` 相同、`runtime`+`agent` 各不相同即可(例:bob/`copy`/open-claw 与 bob/`code`/codex)。

## 第 2 步:要齐接入信息

需要:**接入地址**、**接入密钥**(向管理员要,一次性)。若用户没给,先问这两项。
(注:这是防止他人乱塞数据的写入凭证,不是任何 LLM 用量。)
...
```
装完会把工具放到 `~/.tranfu` 并写入 `TF_SERVER / TF_KEY / TF_OPERATOR`。
...
```
告诉用户去看板确认能看到「operator · 用途 [runtime] 运行中」。
