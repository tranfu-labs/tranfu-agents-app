...
SKILLS 页会低频读取三个只读接口:
- `GET /api/skills?days={7|30|90}`:总览(日趋势、used-only skill 主表、used-only 操作员主表、管理者 Lens、公司库采纳漏斗),响应带服务端 `Asia/Shanghai` `today`;`days=0` 不再支持。管理者 Lens 中"未收录使用占比"按当前窗口 `非公司库 used / 全部 used` 计算,`external` 与 `equipped` 不计入未收录。
- `GET /api/skill/{name}`:单 skill 详情(最近 30 天 used/equipped 分列趋势、runtime/operator 分布、最近记录),响应带服务端 `Asia/Shanghai` `today`。
- `GET /api/operator/{name}`:单操作员详情(used-only 指标、按 skill 分段趋势、skill 排行、runtime 分布、最近记录),响应带服务端 `Asia/Shanghai` `today`;人维度口径是会话×skill 去重,不是实际调用次数。

前端会以服务端 `today` 为右端铺满所选 `Asia/Shanghai` 日窗口;空白天留空槽,今日柱用"进行中"样式,悬停/点击显示当天明细与合计。
