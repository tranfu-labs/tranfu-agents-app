...
SKILLS 页会低频读取三个只读接口:
- `GET /api/skills?days={7|30|90}`:总览(日趋势、used-only skill 主表、used-only 操作员主表、管理者 Lens、公司库采纳漏斗),响应带服务端 UTC `today`;`days=0` 不再支持。管理者 Lens 中"未收录使用占比"按当前窗口 `非公司库 used / 全部 used` 计算,`external` 与 `equipped` 不计入未收录。
