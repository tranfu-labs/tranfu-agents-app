...
> （`recv`），不受跨机时钟漂移影响。具体时间戳保存为 UTC instant;日级统计桶默认按
> `Asia/Shanghai` 自然日切分。客户端无需 NTP 同步也不会污染看板指标。
...
`TF_REPORT_SKILLS=0` 关闭。Claude Code 从 `Skill` 工具调用参数取 skill 名；用户手敲的
...
**本地诊断日志(Hermes 链路):** `shims/tf_hook.py` 为每条 Hermes 事件
(`on_session_start` / `pre_llm_call` / `pre_tool_call` / `post_tool_call` /
`post_llm_call` / `on_session_end`)在 `~/.tranfu/logs/hermes-hook.ndjson` 落一行 NDJSON,
只记字段名 / 工具名 / `skill` 名 / `argv_tail` 摘要 / 上报 `rc`,**不记** stdin 全文、
`tool_input` 非 `name` 字段、shell 命令文本——本地一档与本节"只名不内容"原则一致。
双文件 5MB rotate,总占用上限 10MB;`TF_HOOK_DEBUG=0` 关闭(见 ADR-0022)。
...
- **去重 / 心跳。** 去重键 = `operator + runtime + agent + session_id`。与上一条
  status/step 完全相同、且距上一条已确认心跳不超过 180s 的事件视为纯心跳：只刷新存活时间，
  不产生新历史行、不进 feed。超过 180s 后的同状态恢复会插入新行作为连续计时段起点，旧行
  `last_seen` 保持最后确认心跳；pending batch 中尚未写入 SQLite 的最新心跳也参与判断。
  该恢复边界是服务端内部计时事实，不进入只展示真实状态变化的 feed。
  **去重键包含 `session_id`** —— 同一身份并发多个 session 不会互相吞掉活性。
  服务端可把纯心跳 `last_seen` 进程内合并后批量写入(默认 `TF_HEARTBEAT_BATCH_SECONDS=15` 秒);
  状态/步骤变化、skill、profile、shim 版本变化仍即时落库。
...
- **Agents 指定窗口读侧接口。** `GET /api/agents?w={today|this_week|last_week|7d|14d|30d|90d|custom}`
  返回 `{today,window,summary,comparison,daily,ranking,agents,signals,shim}`。`custom` 另传 Unix 秒级
  `wstart/wend`，按 `Asia/Shanghai` 映射统计日；两端必填、含首尾最多 90 天，起点不得早于服务端
  90 天保留序列。终点可延伸到未来，未来日期按当前时点返回 0，且 `comparison.current.available=false`；
  非法时间戳和非法范围统一返回 `400`。可选 `q/status/signal/sort` 与 Agents 页面使用同一过滤和排序口径，
  `summary.total_agents`、排行、趋势、明细和线索都使用同一过滤集合。
  时长单位为秒；`ranking[]` 排除零时长并按 `active_seconds DESC, key ASC` 排序。
  `ranking[]`、`agents[]` 与 `daily[].segments[]` 均显式返回 `operator`、`agent`、`runtime` 和稳定身份 `key`，
  调用方可直接组装 `${agent} - ${operator}`，无需拆解 key。统计身份仍为 `operator + agent||runtime`。
...
- **skill 使用/装备计数**：当事件带 `skill` 且有 `session_id` 时，服务端记录一行
  ...
  时忽略 `skill` 字段并正常返回。
- **SKILLS 读侧统计接口**：
  - `GET /api/skills?days={7|30|90}` 返回 SKILLS 总览,并携带 `Asia/Shanghai` 当日 `today`。`daily` 与 `table` 只统计 `mode=used`;
    `operator_daily` 与 `operator_table` 同样只统计 `mode=used` 且排除空 `operator`。`days` 只接受 7 / 30 / 90,
    影响 `daily` / `operator_daily` 与 `governance.untracked_usage` 的窗口,不影响主表 7 天 / 30 天 / 累计列,也不影响公司库漏斗。
    `governance.untracked_usage` 计算当前窗口内 `source=非公司库` 且 `mode=used` 的会话×skill 记录数 /
    当前窗口全部 `mode=used` 记录数;`external` 与 `equipped` 不计入未收录。
  ...
  - `GET /api/skill/{name}` 返回单 skill 详情,并携带 `Asia/Shanghai` 当日 `today`:used 指标、equipped 指标、日级 used/equipped 分列序列、
    runtime/operator 分布和最近 50 条会话记录。`equipped` 仅表示 OpenClaw 装备态,不与 used 相加。
  - `GET /api/operator/{name}` 返回单操作员详情,并携带 `Asia/Shanghai` 当日 `today`:used-only 指标、按 skill 分段日级序列、
    skill 排行、runtime 分布和最近 50 条会话记录。人维度口径是会话×skill 去重使用次数,不是实际调用次数;
    equipped 不进入此端点。查无 used 记录返回 404。
  - 上述所有含 Skill 的响应对象都保留 slug `name`/`skill`,并同时返回 `display_name` 与
    `display_name_zh`;批量 payload 另返回 `skill_names` 映射。公司库名称来自 catalog 导出的 `SKILL.md`
    frontmatter,非公司库可由最新 profile 补充。两个 display 字段缺失时服务端回退另一语言或 slug,
    因此调用方可自行按 locale 选名而无需改变稳定 identity。
...
```jsonc
{
  ...
  "skills":   { "local":  [{"name":"prd-to-wireframe",
                                "display_name":"PRD to Wireframe",
                                "display_name_zh":"PRD 转线框","desc":"需求→框架"}],
...
清理台不是 agent 上报协议的一部分,但由同一个服务端提供。它用于删除生产中的测试/错误数据,入口为
`/admin` 与 `/api/admin/*`。

### 鉴权
- 所有 `/api/admin/*` 请求必须带 `X-TF-Admin-Key`,且等于服务端 `TF_ADMIN_KEY`;兼容路径
  `DELETE /v1/events` 也使用同一管理钥匙,不得使用上报钥匙删除数据。
- `TF_ADMIN_KEY` 未配置时管理接口一律 `403`。它与上报用 `TF_KEY` 隔离,不会通过 `install.sh` 分发给成员。
- 被拒的管理请求写入 `admin_audit(action=denied)`,审计里只记录 key 的哈希短码与来源 IP,不记录明文钥匙。

### 选择器
`POST /api/admin/preview` 与 `DELETE /api/admin/data` 都接收 `targets[]`;每个 target 是以下之一:

```jsonc
{ "session_ids": ["s1", "s2"] }                 // 精确 session_id
{ "operator": "alice", "agent": "code", "runtime": "codex" } // agent/runtime 可选
{ "before_day": "2026-06-01", "operator": "alice" }          // 必须带 operator
{ "skill": "typo-skill" }                       // 精确 skill 名
```

`cascade_children=true` 时,会沿 `parent_session_id` 递归删除子会话。`skill` 选择器只删除
`skill_uses` 与重算 `skills_seen`,不删除 `events`。

### 预览即承诺
- `POST /api/admin/preview` 是 dry-run,返回各表将删行数、活跃会话、受影响 operator、`skills_seen.first_day`
  漂移、将清掉的身份卡,以及 `preview_token`。
- `DELETE /api/admin/data` 必须回带该 `preview_token`;服务端重新解析选择器,集合不一致时返回 `409`,要求重新预览。
- 活跃会话默认拒删,须显式 `force=true`。单次影响超过 `TF_ADMIN_MAX_ROWS`(默认 200)或跨多个 operator 时,
  须带 `confirm_count=<实际行数>`。

### 回收站与恢复
- 删除是硬删,但删除前会把源头表 `events / skill_uses / profiles` 的原文快照进 `admin_trash`;若
  `revoke=true`,同批次还会快照被注销的 `operators` 绑定。一批次一个 `batch_id`;派生表
  `skills_seen / identities` 不快照,删除和恢复后都重算。
- `GET /api/admin/trash` 列最近批次;`POST /api/admin/restore {"batch_id":"..."}` 整批恢复,逐表返回
  `inserted / skipped`。`events` 恢复不复用旧自增 id。
- `admin_trash` 按 `TF_TRASH_DAYS`(默认 30)过期真删;`admin_audit` append-only 保留
  `delete / restore / purge_trash / denied / retention_prune`。
