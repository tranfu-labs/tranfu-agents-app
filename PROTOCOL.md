...
```jsonc
{
  ...
  "skill":      "openai-docs",      // optional. 本次工具调用用到的 skill 名；不含参数或内容
...
- **skill 使用计数**：当事件带 `skill` 且有 `session_id` 时，服务端记录一行
  `session_id × skill`；同一会话重复使用同一 skill 只算一次，即使第二次事件命中心跳去重也会先处理。
  无 `session_id` 时忽略 `skill` 字段并正常返回。
...
### 数据保留
事件表只保留 **90 天**窗口；更早的历史按天聚合后删除。`skill_uses` 表是会话×skill 的去重统计源，
