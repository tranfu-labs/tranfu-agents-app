...
- **skill 使用/装备计数**：当事件带 `skill` 且有 `session_id` 时，服务端记录一行
  ...
  时忽略 `skill` 字段并正常返回。
- **SKILLS 读侧统计接口**：
  - `GET /api/skills?days={7|30|90|0}` 返回 SKILLS 总览。`daily` 与 `table` 只统计 `mode=used`;
    `days` 只影响 `daily` 的窗口,不影响主表 7 天 / 30 天 / 累计列,也不影响公司库漏斗。
  ...
  - `GET /api/skill/{name}` 返回单 skill 详情:used 指标、equipped 指标、日级 used/equipped 分列序列、
