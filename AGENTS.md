...
- Agents 视图约束:顶部控制条必须包含操作员/运行终端视角切换与 Skills 同款时间窗(默认今天);时间窗变化标题必须从同一窗口 i18n label 派生。时间窗变化与稳定摘要必须合并为单一八卡网格(活跃 Agent、活跃时长、Agent 总数、操作员数、当前在线、本周活跃、运行质量、待处理),不得另留第二摘要 frame;Agents 标记与样式类必须独立,不得依赖 `.skills-*` 展示类。每卡入口必须落到真实趋势/排行/明细/筛选动作并保留无关 URL 范围。问题线索使用紧凑事实条;桌面主分析区左排行、右趋势近等宽,`≤1080px` 单列。Agents 趋势直接消费 `resolveSkillsChartLayout`:短窗铺满、长窗仅图内滚并优先定位当前指标最后一个非零日期、today 正值为紧凑单日 plot、当前指标全零为无轴 Empty;操作员/运行终端视角同时驱动趋势每日堆叠分组,按当前指标保留 Top 8 + 其他,缺失分组显示未分配,tooltip 展示分组值并同时保留总人数/总时长,日期槽使用 roving focus;切换视角、时间窗或指标必须重置焦点、tooltip 与滚动位置。Agent 明细必须为响应式表格,与顶部 `q/status/signal/rt/op` 和 `w/wstart/wend` 使用同一可见集合及窗口统计,窗口时长/天数从 `active_days` 重算,质量明确为累计口径;桌面完整表格、平板仅表格盒内横滚、手机压缩为摘要行,整行键盘可达下钻。当前/上一窗口只有在 `agent_overview.days` 完整覆盖全部统计日时才可用于变化与分析,不得以部分重叠冒充完整窗口。手机真实 DOM 顺序为控制摘要 → 问题线索 → Agent 明细 → 八卡 → 排行 → 趋势,页面根不得横向滚动。
