...
- `frontend/` — React + TypeScript 看板前端(Vite 构建);FastAPI 在 `/` 与 SPA 深链提供 `frontend/dist`。
- `shims/` — 各客户端上报工具:
...
# Coolify 部署
# 使用根目录 compose.yml;Docker 多阶段会构建 frontend 并复制 dist;给 server service 配 Domain: https://你的域名:8788
