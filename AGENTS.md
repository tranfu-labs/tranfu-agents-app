...
- `/api/state` 与 `/api/state/stream` 是看板状态读路径,服务端必须做进程内 TTL 缓存(默认 `TF_STATE_TTL=1.5` 秒)与 single-flight 重算保护;前端优先 SSE,失败才回退 adaptive polling。快照可带 `agent_overview`(90 天 Agents 活跃/质量聚合),但不得另起 Agents 轮询源。
