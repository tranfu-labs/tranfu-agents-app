...
curl 'http://localhost:8788/api/agents?w=7d' | head     # Agents 近 7 天排行/统计 JSON
...
- Skill 统计口径:事件可选 `skill`(仅 Skill 名)+ 可选 `skill_mode ∈ {used,equipped}`;服务端写 `skill_uses`(一行=会话×Skill×mode,`(session_id, skill, mode)` 幂等,长期保留)。端点:`/api/state.skills`(兼容排行)、`/api/skills?days={7|30|90}`(旧兼容总览)、`/api/skills?w={today|this_week|last_week|7d|14d|30d|90d|custom}`(新版总览,skill/operator 两套 used-only 聚合,含 `Asia/Shanghai` `today`;按人聚合可用 `rt/src` 继承观察范围;`scope=new` 进入当前窗口历史首次 used 的可行动名单;`published_skills[]`/`current_published_skill_count` 使用 catalog `published_at` 统计当前窗口 `own|meta` 新发布 skill,发布但未使用也计入,`external` 不计入)、`/api/skills/evidence?kind=...`(当前窗口证据页 payload,used-only,支持 total/untracked/coverage/operators/avg_per_session/idle/unused_ratio/zero_install/top3/runtime/source)、`/api/skill/{name}`(used/equipped 分列,含 `Asia/Shanghai` `today`)、`/api/operator/{name}`(按人 used-only,空 operator 与 equipped 不计入)。`/api/state.leverage.assets` 与顶部 `N Skill 资产` 使用 used-only distinct skill;`skills_week` 与顶部 `+N 7天新发现` 使用当前 7 天窗口内历史首次 used skill;installed/profile-only/equipped-only 不计入 nav 数字。`skills_seen` 只保留为内部发现/安装痕迹,不得作为 nav 展示事实源。默认上报,本机 `TF_REPORT_SKILLS=0` 关闭;不得上报 Skill 参数/prompt/代码/输出。
  ...
  的静态字符串 `cmd`;字符串/注释伪调用、动态命令、非 `cmd` 字段与 `apply_patch` 不得采集。
  不提供或执行批量历史回填;续聊旧会话在正常完整重扫时允许自然补记。
- 前端 React + TypeScript SPA;生产产物由 Docker/CI 构建,**仓库不提交 `frontend/dist`**。Agents 列表 `/agents` 是与 Skills 视觉语法对齐但业务组件独立的运营 dashboard:控制条(`q/status/signal/w/wstart/wend/sort`) → 单一八卡时间窗变化/快照 → 问题线索 → Agent 运行时长排行与按 Agent 分组的每日时长趋势 → 响应式 Agent 明细表,整行下钻 `/agent/:key`;页面独立请求 `/api/agents`,先渲染 loading skeleton,再用服务端同一窗口 payload 的 `summary/comparison/daily/ranking/agents/signals` 渲染全部统计区块,不得被全局 `/api/state` 首包阻塞。`ranking[]`、`agents[]` 与每日 Agent 分段必须显式带 `operator`,供页面及其它消费者组装身份信息;筛选状态不得持久化。深浅主题为三态 `system` / `light` / `dark`,用 `:root[data-theme]` CSS 变量驱动并设置 `color-scheme`;仅主题模式可用 localStorage key `tf-theme-mode` 保存 `system|light|dark`,其它前端状态仍不得持久化(见 ADR-0023)。品牌红 `--brand`(占位 `#ec1c2b`;实际值已挖出三档红 `#bd0d1c`/`#f42631`/`#ff2f37`,见 `frontend/public/favicon.svg`,CSS 占位是否切换走单独决策);logo 为内联红色 symbol。SKILLS 总览是证据导向 dashboard:控制条(含视角切换、时间窗、搜索、runtime、来源、Top N、隐藏 0 使用,不含环比开关) → 当前时间窗变化(标题从时间窗 i18n label 派生,每格 icon 进记录、短结论、不铺长 skill 名单,previous-window delta 默认开启;Skill 视角第 4 格为新增发布 Skill,跳 `/skills/new`) → 问题线索/按人使用线索(只显示事实值+记录 icon,不显示`看名单/看证据`等动作文案,不露具体 skill 例子,不做 KPI 评分;Skill 视角新增发布 Skill 入口跳 `/skills/new`;记录 icon 默认浅灰,hover/focus 才高亮) → 主分析区(排行 Bar/操作员排行 + 每日趋势图,按窗口长度切布局) → 待处理线索治理行(每类独立区块) → 来源/runtime Donut → 明细表 + Skill 抽屉 → 下沉公司库漏斗;`/skills` 首屏核心文案和时间窗口 label 必须随中英文切换,桌面/平板搜索字段 label+input 保持同一行,KPI 核心数值与记录 icon 保持同一行。`/skills`、`/skills/new`、`/skills/evidence`、`/skills/clues/:kind`、`/skill/:name` 与 `/operator/:name` 不得被全局 `/api/state` 首包阻塞,应先渲染自身 loading/skeleton 并并行请求 SKILLS API;刷新或返回页面可先展示同 URL 已校验旧 payload 作为过渡态,但后台仍必须向服务端 revalidate。桌面 `7d`/`14d` 及以下短窗口下排行和每日趋势左右并列,`30d` 及以上长窗口下二者上下堆叠。手机 `/skills` 控制条默认折叠为一行摘要,首屏先露问题线索和待处理线索,不先铺完整筛选表单;`/skills` 无窗口参数时前端默认 `w=7d`,图表轴使用服务端返回的 `window.start..window.end`,只有窗口右端等于 `today` 才标记"今日进行中";短窗口趋势图填满自身面板可视宽度且限制柱体最大宽度,长窗口在 `.chart-box` 内部横滚并默认显示最新日期。旧 `lens` 参数保留 no-op;未收录使用占比由当前时间窗变化、问题线索、待处理线索共同呈现,口径为当前 `w/days` 窗口内 `source=非公司库` 且 `mode=used` 的会话×skill 记录数 / 当前窗口全部 `mode=used` 记录数,`external` 与 `equipped` 不计入。`/skills/new` 继承当前窗口,展示 catalog `own|meta` 中按 `published_at` 落入窗口的新发布 skill 列表,发布但未使用也保留名单。`/skills/evidence` 继承当前窗口和筛选,用紧凑上下文摘要展示原始 used 记录;有 records 的 kind 第一屏先露 records 表,`Top skills / Top operators` 只作辅助分组,冲突 source 筛选由后端返回 `ignored_filters`。`/skills/clues/untracked|idle|zero-install` 是待处理线索三类详情页:旧 `/skills/evidence?kind=untracked|idle|zero_install` 兼容跳转到新路由;untracked 第一屏先露 Top Operators 并显示 `records/total · percent`,带 `skill=` 时隐藏 Top Skills;idle 第一屏展示 skill、安装人数、安装者、上次使用,不展示 Top Skills / Top Operators;zero-install 第一屏展示零装机名单,不展示 Top Skills / Top Operators;筛选 chip 必须显示人话,不得露 `window_start`/`window_end`/`src: non_catalog`。待处理线索 row 必须分模板,分组摘要用明文总数而非 `8/48`,主操作为查看图标 + 文字 `忽略`,不得用 `×` 忽略。按人视角操作员排行继承当前 `w/days` 与 `runtime/source`,不继承 skill 搜索词、Skill Top N、隐藏 0 使用或选中 skill;主排序使用当前窗口 `sessions_window`。SKILLS 统计域专用断点为桌面 `>1080px`、平板 `601px-1080px`、手机 `≤600px`,平板/手机主内容单列且页面根不得横向滚动,手机排行/最近记录/记录表用摘要行。 Skill 总览明细行整行可点打开抽屉且键盘可达,抽屉内显式按钮跳详情,并展示当前时间窗触发、上期变化、活跃者、装机数、14/30/90 趋势、runtime、使用操作员 Top、装备但未使用差集与最近 5 次;按人主榜与操作员详情 Skill 排行整行跳详情;最近记录表和无下钻目标的记录行不可点。时间列按浏览器本地时区展示 `first_seen`(本地今天内相对时间,昨天显示`昨天 HH:mm`,近 7 天显示星期+时刻,今年更早显示`MM-DD HH:mm`,跨年显示`YYYY-MM-DD HH:mm`,hover 显示完整本地绝对时间+时区;缺失 `first_seen` 时按 `Asia/Shanghai` 统计 `day` 显示今天/昨天/星期/MM-DD/YYYY-MM-DD,hover 保留原始日期)。
- Agents 视图约束:顶部控制条固定为全部 Agent、按运行时长统计,只保留搜索、状态、时间窗(默认今天)与排序;操作员和运行终端不得作为筛选条件,底部明细必须显示操作员列但不得显示运行终端列,旧 `rank/rt/op` 只作 no-op 并从 URL 清理。时间窗变化标题必须从同一窗口 i18n label 派生。时间窗变化与稳定摘要必须合并为单一八卡网格(总运行时长、平均运行时长、活跃 Agent、Agent 总数、当前在线、本周运行时长、累计运行质量、待处理),不得另留第二摘要 frame;包括 `total_agents` 在内的窗口统计必须与当前 `q/status/signal` 使用同一可见集合。Agents 标记与样式类必须独立,不得依赖 `.skills-*` 展示类。每卡入口必须落到真实趋势/排行/明细/筛选动作并保留无关 URL 范围。问题线索使用紧凑事实条;桌面主分析区左 Agent 运行时长排行、右 Agent 运行时长趋势近等宽,`≤1080px` 单列。Agents 趋势直接消费 `resolveSkillsChartLayout`:单日正值按 Agent 时长显示同风格环形扇形图,多日短窗铺满按 Agent 分段的堆叠柱,长窗仅图内滚并优先定位最后一个非零日期,全窗时长为零时显示无轴 Empty;分组按运行时长保留 Top 8 + 其他,tooltip 展示各 Agent 时长并同时保留总人数/总时长,单日与逐日日期槽均使用 roving focus;切换时间窗必须重置焦点、tooltip 与滚动位置。Agent 明细必须为响应式表格,与顶部 `q/status/signal` 和 `w/wstart/wend` 使用同一服务端可见集合及窗口统计,质量明确为累计口径;桌面完整表格、平板仅表格盒内横滚、手机压缩为含操作员的摘要行,整行键盘可达下钻。当前/上一窗口只有在 `/api/agents` 明确标记完整时才可用于变化与分析,未来日期可返回零值但 current comparison 必须标记不可用,不得以部分重叠冒充完整窗口。custom 半填写和完整 query 切换必须保留真实控制条并只 skeleton 数据区;失败时不得在新 URL 下继续展示旧 payload。手机真实 DOM 顺序为控制摘要 → 问题线索 → Agent 明细 → 八卡 → 排行 → 趋势,页面根不得横向滚动。
- Agents 排行名称、进度条、运行时长与窗口元信息使用跨行稳定列轨道,同一断点所有进度条从同一水平位置起始;同名但不同身份的 Agent 必须稳定区分,不得跨身份合并时长;明细相对时间列必须明确标为“距上次活跃 / Time since last active”。
- Token Usage `/token-usage` 的全部可见筛选与排序使用 URL query `w/wstart/wend/g/kind/model/risk/topn/q/hz/sort/dir`,变化使用 replace,默认值无需冗余写入 URL;刷新、复制与历史导航必须恢复状态。`w=custom` 允许 `wstart/wend` 逐项保留,两端有效后才请求;详情抽屉、选中 KEY 与忽略风险保持临时 state,不得写 URL 或浏览器存储。
...
## 线框图
默认生成 `docs/wireframes/`(字符图线框,对齐页面信息架构与版式)。本项目**有界面**(`frontend/` SPA),按 `docs/wireframes/AGENTS.md` 维护:每个真实路由在 `docs/wireframes/pages/` 对应一页(现覆盖 `/`·`/agents`·`/agent/:key`·`/skills`·`/skills/new`·`/skills/evidence`·`/skills/clues/:kind`·`/token-usage`·`/skill/:name`·`/operator/:name`),页面流转记在 `docs/wireframes/flow.md`;新增/改路由时同步增改对应页与流转图。(若项目转为无界面的工具/库/CLI/SDK,则删除整个 `docs/wireframes/` 目录与本节。)
