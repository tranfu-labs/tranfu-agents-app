...
- Agents 视图约束:顶部控制条必须包含 Runtime/操作员视角切换与 Skills 同款时间窗(默认今天);时间窗变化区域必须像 Skills 统计一样使用独立卡片展示当前/上一同长度窗口的活跃 Agent、活跃时长变化，以及当前在线和运行质量快照;问题线索使用紧凑事实条;桌面主分析区左侧排行、右侧趋势,`≤1080px` 单列且页面根不得横向滚动。
