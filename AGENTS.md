...
- Agents 视图约束:顶部控制条必须包含 Runtime/操作员视角切换与 Skills 同款时间窗(默认今天);时间窗变化与稳定摘要必须合并为单一八卡网格(活跃 Agent、活跃时长、Agent 总数、操作员数、当前在线、本周活跃、运行质量、待处理),不得另留第二摘要 frame;每卡入口必须落到真实趋势/排行/明细/筛选动作并保留无关 URL 范围。问题线索使用紧凑事实条;桌面主分析区左排行、右趋势近等宽,`≤1080px` 单列。Agents 趋势直接消费 `resolveSkillsChartLayout`:短窗铺满、长窗仅图内滚、today 正值为紧凑单日 plot、当前指标全零为无轴 Empty,tooltip 同时显示人数/时长且日期槽使用 roving focus。手机真实 DOM 顺序为控制摘要 → 问题线索 → Agent 明细 → 八卡 → 排行 → 趋势,页面根不得横向滚动。
