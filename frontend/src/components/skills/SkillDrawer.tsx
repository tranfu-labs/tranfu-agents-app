...
export function SkillDrawer({ name, search, onClose, t }: { name: string; search: string; onClose: () => void; t: (key: string) => string }) {
        ...
        {detail ? (
          <>
            <div className="skills-drawer-kpis">
              <div className="stat"><div className="v">{detail.metrics?.sessions_30d || 0}</div><div className="l">30d 触发</div></div>
              <div className="stat"><div className="v">{detail.metrics?.users_30d || 0}</div><div className="l">活跃者</div></div>
              <div className="stat"><div className="v">{detail.metrics?.sessions_total || 0}</div><div className="l">累计</div></div>
              <div className="stat"><div className="v">{detail.metrics?.equipped_30d || 0}</div><div className="l">装备态</div></div>
            </div>
            <DetailTrend detail={detail} t={t} />
