...
export function AgentKpiGrid({ comparison, summary, totalAgents, attention, windowLabel, onAction, t }: {
  ...
  return (
    <section className="frame skills-kpi-frame agents-window-frame">
      ...
      <div className="skills-kpi agents-window-kpi">
        {cards.map((item) => (
          <div className={`stat skills-kpi-card agent-kpi-card agent-kpi-${item.key}`} key={item.key}>
            <div className="skills-kpi-top">
              <div className="v">{item.value}</div>
              <button
                type="button"
                className="evidence-icon-link agent-kpi-action"
