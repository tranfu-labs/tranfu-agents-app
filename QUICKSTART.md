...
Open Claw / Hermes / 自写脚本都行;Codex 也可以这样做一次性包装。`started` 时**自动探测并上报 profile**,
中途心跳,结束报完成/失败:
...
Claude Code 的 Skill 工具调用会默认附带 skill 名用于团队排行统计;只记录名称,不记录参数或内容。
如需关闭,在本机环境里设置 `export TF_REPORT_SKILLS=0` 后重启 Claude Code / Codex。
...
> Skill 使用排行默认只统计 skill 名。如不希望参与统计,设置 `export TF_REPORT_SKILLS=0`。
