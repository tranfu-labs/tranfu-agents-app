...
安装器会按服务端 manifest 全量下载并校验 shim,成功后写入 `~/.tranfu/manifest.json` 并开启
shim 自更新。以后 Claude Code / Codex / Hermes 在会话开始时会后台检查新版 shim,失败静默、不影响 agent;
如果本地记录版本一致但文件缺失或哈希不符,也会自动补齐。要关闭可在对应
...
Hermes 的 `skill_view(name)` 工具调用会默认附带 skill 名用于团队 SKILLS 统计;只记录名称,
...
> SKILLS 统计页默认只统计 skill 名:Claude Code 取自 `Skill`,Hermes 取自 `skill_view`,
> Codex 取自本机会话文件里的已装 `SKILL.md` 读取信号;OpenClaw 取自 prompt 注入块并标为
> `equipped` 装备态。SKILLS 总览只排 `used`;装备态只在单 skill 详情里展示,不与使用态相加。
> 如不希望参与统计,设置 `export TF_REPORT_SKILLS=0`。
...
- **SKILLS**:使用过的 Skill 会出现在总览主表;每日使用图按 7/30/90 天 `Asia/Shanghai` 统计时间窗铺满,悬停可看当天明细与合计。点进单个 Skill 能看到最近 30 天趋势和 runtime/operator 分布。OpenClaw 装备态只在详情里显示 `equipped`。
- 卡片或详情里能看到 shim 短版本;若显示"旧 shim",说明该机器尚未自动更新成功或还没重跑新版安装器。

没出现?看板超过 3 分钟没收到心跳会判为掉线(空闲),并把本次连续运行时长停在最后一次心跳;
重新跑任务会从恢复时刻开启新计时段。
