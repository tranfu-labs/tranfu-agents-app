...
Open Claw / 自写脚本都行;Hermes 也可以这样临时包装,但要实时工具步骤与 Skill 统计请看 C。Codex 也可以这样做一次性包装。`started` 时**自动探测并上报 profile**,
中途心跳,结束报完成/失败:
...
> Codex 取自本机会话文件里的已装 `SKILL.md` 读取信号。如不希望参与统计,设置 `export TF_REPORT_SKILLS=0`。
