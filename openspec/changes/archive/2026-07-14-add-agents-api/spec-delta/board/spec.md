...
- `w=custom` 的 `wstart/wend` 为 Unix 秒，按 `Asia/Shanghai` 映射统计日；非法时间戳、缺参数、倒序、超过 90 天或起点早于当前 90 天可用序列均返回 `400`。终点可延伸到未来，未来日期按当前时点返回 0，但 `comparison.current.available=false`。
...
- `agents[]`、`daily[]`、`summary`、`comparison` 和 `signals` 必须使用同一组经 `q/status/signal` 过滤的身份卡片与同一窗口，不得混用全量集合或浏览器二次统计；`summary.total_agents` 也不得绕过过滤集合。
...
- `/agents` 必须独立请求 `/api/agents`，不得等待全局 `/api/state` 首包后才挂载；页面首次加载和 query 变化期间显示与当前信息架构对应的 skeleton。custom 半填写态必须保留真实控制条及日期输入，只替换数据区；失败时即使存在旧 payload 也必须显示可重试错误态，不得在新 URL 下呈现旧统计。
...
- `GET /api/agents?w=custom&wstart=<上海日A内instant>&wend=<上海日B内instant>` → 起止日分别为 A/B；非法时间戳、跨越 91 个统计日或起点早于 90 天保留序列时返回 `400`；未来 B 的日期槽按当前时点为 0，当前 comparison 不可用。
