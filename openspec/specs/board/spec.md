...
- 操作员详情与单 skill 详情的"最近记录"列表,时间列须以浏览器本地时区展示 `first_seen`:
  ...
  浏览器本地昨天及更早显示本地绝对时间 `YYYY-MM-DD HH:mm:ss`;鼠标悬浮时间单元格时,title 显示本地绝对时间与浏览器时区名
  (时区名不可得时省略)。`first_seen` 缺失时回退原始 UTC 日期 `day`,不得把 date-only 值强行按浏览器时区换算。
...
- 操作员详情/skill 详情"最近记录"构造仅有 `day` 无 `first_seen` 的记录 → 回退显示原始日期。
