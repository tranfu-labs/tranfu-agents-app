...
8. **自更新安全边界**:`tf_selfupdate.py` 只在 `SessionStart` / `on_session_start` 后台触发;必须先下载到 staging,
   校验 sha256,`.py` 通过 `py_compile`,全部通过后才替换正式文件;失败静默且保留旧文件。本地 manifest
   与服务端版本一致但目标文件缺失或哈希不符时,必须补齐该文件。`TF_AUTO_UPDATE=0` 完全关闭。
9. Claude Code / Codex / Hermes 的新 shim 在下一次 hook 触发时生效;OpenClaw 插件文件可被刷新,但 JS 常驻加载需重启 OpenClaw。
...
- `TF_AUTO_UPDATE=0` 时触发 SessionStart 不启动更新;服务端不可达或坏包时旧 shim 保留。
