...
5. 三条接入路径并存:`tf-run`(任意 CLI)、Claude Code 钩子(`tf_hook.py`,见 ADR-0009)、MCP reporter(桌面/黑盒)。
6. **同一 agent 始终用同一套 `operator/runtime/agent`**;漏掉 `--agent` 会退化为按 runtime 显示(产生独立卡)。

## 可验证行为
- `curl $SERVER/install.sh` 出脚本;`curl $SERVER/shims/tf_hook.py` 出文件;`curl $SERVER/shims/../server/app.py` 返回 404。
