...
### M2 — 看板前端 (`frontend/`)
- **职责**:优先通过 `/api/state/stream` SSE(失败时回退 `/api/state` adaptive polling)渲染 Pods 看板 / Agents 运营列表 / 治理详情;Agents 使用 `/api/state.agent_overview` 与身份卡片 `active_days` 做 90 天活跃/质量分析、按操作员/运行终端展示单日环形分布或多日堆叠趋势及随控制条窗口重算的明细表;低频读取
