...
### M2 — 看板前端 (`frontend/`)
- **职责**:优先通过 `/api/state/stream` SSE(失败时回退 `/api/state` adaptive polling)渲染 Pods 看板 / Agents 运营列表 / 治理详情;Agents 使用 `/api/state.agent_overview` 做 90 天活跃/质量/Runtime/操作员分析;低频读取
