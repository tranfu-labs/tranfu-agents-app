...
- **从源文件补采,而非 hook 内识别命令**:Codex 把每个会话写成磁盘 rollout
  ...
  且为未来从源文件提取更多信号留同一入口。
- **只认强信号**:仅计入 `payload.type=="function_call"` 行中、读取了已装目录
  (`.codex/` 或 `.claude/` 点目录前缀)下 `skills/<名>/SKILL.md` 的记录。提示词点名、技能目录注入、
  工具输出回显、`apply_patch` 改写、作者仓库散落的 SKILL.md 一律不计。宁缺毋错,与现有统计口径一致。
